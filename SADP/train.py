...
def main():
    ...
    train_low_data_names = './data/low-pig/train'
    TrainDataset = MemoryFriendlyLoader(img_dir=train_low_data_names, task='train')


    test_low_data_names = './data/low-pig/test'
    ...
    for epoch in range(args.epochs):
        ...
        # torch.save(model, os.path.join(model_path, 'weights_%d.pt' % epoch))
        utils.save(model, os.path.join(model_path, 'weights_%d.pt' % epoch))

        if epoch % 1 == 0 and total_step != 0:
            logging.info('train %03d %f', epoch, loss)
            # model = utils.load(model, os.path.join(model_path, 'weights_%d.pt' % epoch))
            # model = torch.load(os.path.join(model_path, 'weights_%d.pt' % epoch))
            model.eval()
            with torch.no_grad():
